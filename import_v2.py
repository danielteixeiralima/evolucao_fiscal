# -*- coding: utf-8 -*-
"""
Importador V2 - Usa modelos SQLAlchemy puros (sem Flask).
"""
import argparse
import sys
...
class ImporterV2:
    ...
    def get_or_create_company(self, code):
        ...
        if not company:
            company = Company_V2(code=code, name=f"Empresa {code}")
    ...
    def get_or_create_branch(self, company, code):
        ...
        if not branch:
            branch = Branch_V2(company_id=company.id, code=code, name=f"Filial {code}")
    ...
    def import_movements(self, start_date, end_date, company_code, branch_code, page_size=50, max_pages=1000):
        ...
        print(f"\n{'='*72}")
        print(f"🚀 Importando da API TOTVS (V2)")
        print(f"{'='*72}")
